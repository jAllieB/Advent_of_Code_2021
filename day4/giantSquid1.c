...
int main () {
	...
	char *s, *tok;
	int *drawOrder, bestBoard[5][5], currBoard[5][5], bestTurns = 1000, currTurns, points, i, j;
	IS is;
	...
	while (get_line(is) >= 0) {
		...
		/* if this is the first round or this is the best board */

		if (currTurns < bestTurns) {

			/* set best board and turns to this board and turns */

			bestTurns = currTurns;
			for (i=0; i<5; ++i)
				for (j=0; j<5; ++j)
					bestBoard[i][j] = currBoard[i][j];
		}
	...
	points = bingo(drawOrder, bestBoard, 1);	

	
	/* printing puzzle info */

	printf("Draw Order: ");
	for (i=0; i<bestTurns; ++i) printf("%d ", drawOrder[i]);
	printf("\n");
	...
	printf("Lowest Number of Turns: %d\nWinning Draw: %d\nSum of Unmarked Numbers: %d\nPoints: %d\n",
		       bestTurns, drawOrder[bestTurns-1], (points / drawOrder[bestTurns-1]), points);
